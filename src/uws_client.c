...
void uws_client_destroy(UWS_CLIENT_HANDLE uws_client)
{
    ...
    else
    {
        ...
        if (uws_client->request_headers != NULL)
        {
...
static char* get_request_headers(MAP_HANDLE headers)
{
    ...
    if (headers == NULL)
    {
        ...
        {
            LogError("Failed allocating empty string for request headers");
        }
        else
        {
            result[0] = '\0';
...
int uws_client_set_request_header(UWS_CLIENT_HANDLE uws_client, const char* name, const char* value)
{
    ...
    else if (uws_client->request_headers == NULL &&
        (uws_client->request_headers = Map_Create(NULL)) == NULL)
    {
